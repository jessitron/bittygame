...
object JessLife {
  ...
  private val pleaseTheTeachers = Opportunity.printing("do all my homework",
    "Your teachers love you.").andIncrease(grades.name).onlyIf(NotHas(collegeEnrollment))
  ...
  private val amdocs = Opportunity.printing("get a programming job",
    ...
    """).onlyIf(Has(loveOfProgramming)).andProvides(steadyIncome)
  ...
  onlyIf(Has(steadyIncome)).andProvides(chanceToMeetTed)
  ...
  onlyIf(MustBeHighEnough(programmingCred.name, 2)).andIncrease(programmingCred.name)
  ...
  private val speakAtStrangeloop = Opportunity.victory("speak at StrangeLoop",
    ...
    behindObstacle(MustBeHighEnough(programmingCred.name, 4), "Your abstract is not accepted.").
    behindObstacle(MustBeHighEnough(publicSpeaking.name, 5), "Your abstract is not accepted.")
  ...
  val scenario = Scenario("jesslife",
    ...
    welcome, Seq(grades, programmingCred, publicSpeaking), Seq(loveOfProgramming, steadyIncome, collegeEnrollment, partTimeJob, house, husband, children, speakingMentor))
}
